...
Request.prototype.request = function(){
  ...
  try {
    const query = qs.stringify(this.qs, {
      ...
      encoder: backtickAwareQueryEncoder
    });
    if (query) {
      this.qs = {};
...
/**
 * An encoder function for qs.stringify that's aware of the fact that we
 * want to leave unencoded backticks as-is, per the WHATWG spec.
 *
 * @param  {any} v The value getting serialized for the querystring
 * @param  {Function} origQsEncode The default encoder from qs
 * @return {String} encoded value
 */
function backtickAwareQueryEncoder(v, origQsEncode) {
  return typeof v !== 'string'
    ? origQsEncode(v)
    : origQsEncode(v).replace(/%60/g, "`");
}
